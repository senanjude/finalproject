...
def get_ratio_digits_letters(url):
      num_digits = len(list(filter(lambda x:x.isnumeric(),url)))
      num_alphabets = len(list(filter(lambda x: x.isalpha(),url)))
      try:
        ratio = num_digits/num_alphabets
        return ratio 
      except Exception as e:
        return 0 
